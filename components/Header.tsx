...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100 shadow-sm">
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="flex justify-between items-center h-20">
          ...
          <nav className="hidden md:flex items-center gap-8 font-medium text-gray-600">
            ...
            <button onClick={() => scrollTo('scenarios')} className="hover:text-indigo-600 transition-colors">Долги и Банкротство</button>
            <button onClick={() => scrollTo('articles')} className="hover:text-indigo-600 transition-colors">Статьи</button>
          </nav>

          {/* КНОПКА CTA (ВМЕСТО "ВОЙТИ") */}
      ...
      {/* МОБИЛЬНОЕ МЕНЮ (ОТКРЫВАЕТСЯ НА ТЕЛЕФОНАХ) */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-b border-gray-100 absolute w-full shadow-xl">
          <div className="flex flex-col px-4 pt-2 pb-6 space-y-4 font-medium text-gray-600">
            ...
            <button onClick={() => scrollTo('scenarios')} className="text-left py-2 border-b border-gray-50">Долги и Банкротство</button>
            <button onClick={() => scrollTo('articles')} className="text-left py-2 border-b border-gray-50">Статьи</button>
            <button 
              onClick={() => scrollTo('calculator')}
              className="bg-indigo-600 text-white font-bold py-3 rounded-xl mt-4 text-center"
            >
              Рассчитать ставку
            </button>
          </div>
