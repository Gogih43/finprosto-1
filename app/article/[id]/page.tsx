import { articlesData } from '../../../data/articles'; // проверь, правильный ли путь
import { Metadata } from 'next';
import Link from 'next/link';

// === МАГИЯ SEO: АВТОМАТИЧЕСКИЕ МЕТА-ТЕГИ ДЛЯ КАЖДОЙ СТАТЬИ ===
export function generateMetadata({ params }: { params: { id: string } }): Metadata {
  const article = articlesData.find((a) => a.id.toString() === params.id);

  if (!article) {
    return {
      title: 'Статья не найдена | Фин просто',
    };
  }
  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-20 pt-10">
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        {/* Ссылка НАЗАД (Память страницы работает через sessionStorage) */}
        <Link 
          href="/#articles" 
